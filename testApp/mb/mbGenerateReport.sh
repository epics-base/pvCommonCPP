...
# TODO sort its content (stages must follow 0,1,2,3...!)
echo $INPUT_LIST | grep -v merged | cut -d '_' -f 2 | sort -u | xargs -I {} sh -c "cat $MB_OUTPUT_DIR/mb_{}*.csv > $MB_REPORT_DIR/merged_{}.csv"
...
rm STAT_FILE
    ...
    do
        ...
        # TODO
        IGNORE_FIRST_N=0
        mb_stat $FILE -n -s $STAGE -i $IGNORE_FIRST_N | gnuplot -e 'set print "-"; set datafile separator ","; stat "-" using 3 prefix "s0" nooutput; print s0_min, s0_lo_quartile, s0_mean, s0_stddev, s0_up_quartile, s0_max' >> $STAT_FILE
        echo "      done."
    done
